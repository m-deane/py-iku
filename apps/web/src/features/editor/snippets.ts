/**
 * Curated Python snippets for the convert-page editor and the snippet gallery.
 *
 * Every snippet is paraphrased from `py2dataiku/examples/recipe_examples.py`
 * (or `processor_examples.py`) so the conversion engine has a deterministic
 * mapping back to a real RecipeType / ProcessorType. Do not invent new
 * snippets without first adding a matching example to the py-iku library —
 * otherwise users land on a "no recipe detected" failure on first run.
 */

export type SnippetCategory = "pandas" | "numpy" | "sklearn" | "feature-eng";

export interface Snippet {
  id: string;
  name: string;
  description: string;
  code: string;
  /** Free-form labels surfaced by the picker filter. */
  tags: string[];
  /** Coarse grouping used by the gallery view. */
  category: SnippetCategory;
}

export const SNIPPETS: readonly Snippet[] = [
  {
    id: "groupby-agg",
    name: "Read CSV + groupby + agg",
    description:
      "Aggregate transactions by category. Maps to a GROUPING recipe.",
    tags: ["pandas", "groupby", "aggregation", "GROUPING"],
    category: "pandas",
    code: `import pandas as pd

# Load data
df = pd.read_csv('transactions.csv')

# Group by single column with multiple aggregations
category_summary = df.groupby('category').agg({
    'amount': 'sum',
    'quantity': 'mean',
    'transaction_id': 'count',
}).reset_index()
category_summary.columns = ['category', 'total_amount', 'avg_quantity', 'transaction_count']

# Save output
category_summary.to_csv('category_summary.csv', index=False)
`,
  },
  {
    id: "merge-two-dataframes",
    name: "Merge two DataFrames",
    description:
      "Inner join customers and orders on customer_id. Maps to a JOIN recipe.",
    tags: ["pandas", "merge", "join", "JOIN"],
    category: "pandas",
    code: `import pandas as pd

# Load datasets
customers = pd.read_csv('customers.csv')
orders = pd.read_csv('orders.csv')

# Inner join
customer_orders = pd.merge(
    customers,
    orders,
    on='customer_id',
    how='inner',
)

# Save output
customer_orders.to_csv('customer_orders.csv', index=False)
`,
  },
  {
    id: "sort-top-n",
    name: "Sort + select top-N",
    description:
      "Sort by sales then keep the top 10 rows. Maps to a SORT recipe followed by a TOP_N recipe.",
    tags: ["pandas", "sort", "nlargest", "SORT", "TOP_N"],
    category: "pandas",
    code: `import pandas as pd

# Load data
df = pd.read_csv('sales.csv')

# Sort by sales amount descending
sorted_sales = df.sort_values('sales_amount', ascending=False)

# Top 10 by sales amount
top_10_sales = sorted_sales.nlargest(10, 'sales_amount')

# Save outputs
sorted_sales.to_csv('sales_sorted.csv', index=False)
top_10_sales.to_csv('top_10_sales.csv', index=False)
`,
  },
  {
    id: "pivot-melt",
    name: "Pivot / unpivot (melt)",
    description:
      "Reshape a wide monthly metrics table into long format. Maps to a PREPARE recipe with the FOLD_MULTIPLE_COLUMNS processor.",
    tags: ["pandas", "melt", "unpivot", "PREPARE", "FOLD_MULTIPLE_COLUMNS"],
    category: "pandas",
    code: `import pandas as pd

# Load wide data
wide_df = pd.read_csv('monthly_metrics.csv')

# Melt (unpivot) wide -> long format
long_df = pd.melt(
    wide_df,
    id_vars=['product_id', 'product_name'],
    value_vars=['jan', 'feb', 'mar', 'apr', 'may', 'jun'],
    var_name='month',
    value_name='sales',
)

# Save output
long_df.to_csv('monthly_metrics_long.csv', index=False)
`,
  },
  {
    id: "window-rolling",
    name: "Window: rolling mean",
    description:
      "Compute rolling 7- and 30-day metrics on a time series. Maps to a WINDOW recipe.",
    tags: ["pandas", "rolling", "window", "WINDOW"],
    category: "pandas",
    code: `import pandas as pd

# Load time series data
df = pd.read_csv('daily_metrics.csv')
df['date'] = pd.to_datetime(df['date'])
df = df.sort_values('date')

# Rolling window calculations
df['rolling_7d_avg'] = df['value'].rolling(window=7).mean()
df['rolling_30d_sum'] = df['value'].rolling(window=30).sum()
df['cumulative_sum'] = df['value'].cumsum()

# Save output
df.to_csv('metrics_with_windows.csv', index=False)
`,
  },
  {
    id: "sklearn-train-test-logistic",
    name: "Sklearn: train/test split + LogisticRegression",
    description:
      "Split a dataset and fit a logistic regression. Maps to a SPLIT recipe and a sklearn-model handler in py-iku.",
    tags: ["sklearn", "train_test_split", "LogisticRegression", "SPLIT"],
    category: "sklearn",
    code: `import pandas as pd
from sklearn.model_selection import train_test_split
from sklearn.linear_model import LogisticRegression

# Load data
df = pd.read_csv('customers.csv')

# Train / test split (stratified)
train, test = train_test_split(
    df,
    test_size=0.2,
    stratify=df['segment'],
    random_state=42,
)

# Fit a logistic regression on the training set
features = ['age', 'income', 'tenure_months']
model = LogisticRegression(max_iter=1000)
model.fit(train[features], train['churned'])

# Save outputs
train.to_csv('train_set.csv', index=False)
test.to_csv('test_set.csv', index=False)
`,
  },
  {
    id: "filter-rows",
    name: "Filter rows on a condition",
    description:
      "Subset rows where amount > 100. Maps to a PREPARE recipe with a FILTER_ON_VALUE processor.",
    tags: ["pandas", "filter", "boolean", "PREPARE", "FILTER_ON_VALUE"],
    category: "pandas",
    code: `import pandas as pd

# Load data
df = pd.read_csv('transactions.csv')

# Filter to high-value rows
high_value = df[df['amount'] > 100]

# Save output
high_value.to_csv('high_value_transactions.csv', index=False)
`,
  },
  {
    id: "drop-duplicates",
    name: "Drop duplicate rows",
    description:
      "Deduplicate a customer table on email. Maps to a DISTINCT recipe.",
    tags: ["pandas", "drop_duplicates", "DISTINCT"],
    category: "pandas",
    code: `import pandas as pd

# Load data
df = pd.read_csv('customers.csv')

# Drop duplicates by email
unique_customers = df.drop_duplicates(subset=['email'])

# Save output
unique_customers.to_csv('unique_customers.csv', index=False)
`,
  },
  {
    id: "concat-stack",
    name: "Concat / stack two DataFrames",
    description:
      "Stack monthly partitions into a single table. Maps to a STACK recipe.",
    tags: ["pandas", "concat", "STACK"],
    category: "pandas",
    code: `import pandas as pd

# Load partitions
jan = pd.read_csv('sales_jan.csv')
feb = pd.read_csv('sales_feb.csv')

# Stack
all_sales = pd.concat([jan, feb], ignore_index=True)

# Save output
all_sales.to_csv('sales_all.csv', index=False)
`,
  },
  {
    id: "feature-eng-numeric-transform",
    name: "Numeric transforms (round/abs/clip)",
    description:
      "Round amounts, absolute-value deltas, and clip outliers. Maps to a PREPARE recipe with NUMERIC_TRANSFORM processors.",
    tags: ["pandas", "round", "abs", "clip", "PREPARE", "NUMERIC_TRANSFORM"],
    category: "feature-eng",
    code: `import pandas as pd

# Load data
df = pd.read_csv('orders.csv')

# Numeric transforms
df['amount_rounded'] = df['amount'].round(2)
df['delta_abs'] = df['delta'].abs()
df['amount_clipped'] = df['amount'].clip(lower=0, upper=10000)

# Save output
df.to_csv('orders_clean.csv', index=False)
`,
  },
  {
    id: "feature-eng-rename",
    name: "Rename columns",
    description:
      "Standardise column names for downstream pipelines. Maps to a PREPARE recipe with COLUMN_RENAMER processors.",
    tags: ["pandas", "rename", "PREPARE", "COLUMN_RENAMER"],
    category: "feature-eng",
    code: `import pandas as pd

# Load data
df = pd.read_csv('events.csv')

# Standardise names
df = df.rename(columns={
    'usrId': 'user_id',
    'evtTs': 'event_ts',
    'amt': 'amount',
})

# Save output
df.to_csv('events_clean.csv', index=False)
`,
  },
  {
    id: "numpy-vectorized",
    name: "Numpy vectorised features",
    description:
      "Use numpy to compute log/sqrt features. Maps to a PREPARE recipe with NUMERIC_TRANSFORM processors.",
    tags: ["numpy", "log", "sqrt", "PREPARE", "NUMERIC_TRANSFORM"],
    category: "numpy",
    code: `import numpy as np
import pandas as pd

# Load data
df = pd.read_csv('measurements.csv')

# Vectorised numeric features
df['log_value'] = np.log1p(df['value'])
df['sqrt_count'] = np.sqrt(df['count'])

# Save output
df.to_csv('measurements_features.csv', index=False)
`,
  },
];

/** Default snippet shown when the editor first mounts. */
export const DEFAULT_SNIPPET_ID = "groupby-agg";

export const SNIPPET_CATEGORIES: readonly SnippetCategory[] = [
  "pandas",
  "numpy",
  "sklearn",
  "feature-eng",
];

export function getSnippet(id: string): Snippet | undefined {
  return SNIPPETS.find((s) => s.id === id);
}

export function getDefaultCode(): string {
  const fallback = SNIPPETS[0];
  return getSnippet(DEFAULT_SNIPPET_ID)?.code ?? fallback?.code ?? "";
}

export function getSnippetsByCategory(category: SnippetCategory): readonly Snippet[] {
  return SNIPPETS.filter((s) => s.category === category);
}

...
## Confidence shading and source-line attribution (Studio UI)

The LLM path also emits per-step *confidence* and *source-line* metadata that the py-iku Studio canvas turns into a trust signal — the spec is written for trader-engineers who will not deploy a flow they cannot trust. Each step carries (1) a self-rated `confidence` in `[0.0, 1.0]`, (2) a one-sentence `reasoning`, and (3) an inclusive `[start, end]` 1-indexed `source_lines` span pointing back at the originating Python. The Studio recipe-card derives a band from the confidence: `>= 0.85` is "high" and ships unshaded, `0.60–0.84` is "medium" and gains a 2px warn-coloured border plus a ⚠ glyph, `< 0.60` is "low" and the card pulses with a danger-coloured ring so the human reviewer's eye is drawn to it. Recipes from the rule-based path keep `confidence: null` and render an "R" badge in the bottom-left so the two production sources are visually distinguishable. Hovering or focusing the card opens an accessible popover (Tab focuses, Enter opens, Esc closes) with the percentage, the reasoning sentence, and a "Lines X-Y of source ↗" link that drives `monaco.editor.deltaDecorations` to highlight the originating Python lines in the Convert page editor. A summary panel above the canvas counts the bands ("12 recipes converted • 11 high (≥85%) • 1 medium (≥60%) • 0 low") and exposes a global "Show only low-confidence" toggle that dims everything else to 0.3 opacity. The colours are sourced exclusively from the design-system semantic tokens (`--warn-border`, `--danger-border`, `--success-fg`) so the shading tracks light/dark themes automatically — no inline hex anywhere. See the [LLM section of the cheatsheet](appendix-c-cheatsheet.md#llm-path) for the band thresholds at a glance.
